...
//		'contributor' => 'edit_posts',
	...
	/**
	 * Static wrapper for load_modules
	 */
	static function load_modules_instance() {
		$jetpack = Jetpack::init();
		if ( $jetpack && is_object($jetpack) ) {
...
/* Jetpack Options API */
	...
	static function get_option_names( $type = 'compact' ) {
		switch ( $type ) {
		case 'non-compact' :
		case 'non_compact' :
			return array(
	 ...
	 */
	static function get_available_modules( $min_version = false, $max_version = false ) {
		static $modules = null;
	 ...
	 */
	static function get_module_path( $slug ) {
	 ...
	 */
	static function get_active_modules() {
...
class Jetpack_Data {
	 ...
	 * @return object|false
	 */
	static function get_access_token( $args ) {
		if ( is_numeric( $args ) ) {
...
add_action( 'plugins_loaded', array( 'Jetpack', 'load_modules_instance' ), 100 );
